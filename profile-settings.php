...
if (isset($_POST['add_developer'])) {
    ...
    if (empty($developer_name)) {
        $error_message = "Developer name is required.";
    } else {
        ...
        if ($check_result->num_rows > 0) {
            ...
            if ($insert_stmt->execute()) {
                ...
                $error_message = "Error adding developer: " . $conn->error;
            }
            $insert_stmt->close();
        }
        $check_stmt->close();
...
if (isset($_POST['edit_developer'])) {
    ...
    if (empty($developer_name)) {
        ...
        // Check if developer name already exists (excluding this developer)
        $check_stmt = $conn->prepare("SELECT id FROM developers WHERE name = ? AND id != ?");
        $check_stmt->bind_param("si", $developer_name, $developer_id);
        $check_stmt->execute();
        $check_result = $check_stmt->get_result();
        ...
        if ($check_result->num_rows > 0) {
            ...
            // Update developer
            $update_stmt = $conn->prepare("UPDATE developers SET name = ? WHERE id = ?");
            $update_stmt->bind_param("si", $developer_name, $developer_id);
            
            if ($update_stmt->execute()) {
                $success_message = "Developer updated successfully.";
                // Refresh the page
                header("Location: settings.php?tab=developers");
                exit();
            } else {
                $error_message = "Error updating developer: " . $conn->error;
            }
            $update_stmt->close();
        }
        $check_stmt->close();
...
if (isset($_GET['delete_developer'])) {
    ...
    if ($model_count > 0) {
        $error_message = "Cannot delete developer with associated project models. Please delete the models first.";
    } else {
        // Delete developer
        ...
        if ($delete_stmt->execute()) {
            $success_message = "Developer deleted successfully.";
            // Refresh the page
            header("Location: settings.php?tab=developers");
            exit();
        } else {
...
if (isset($_POST['add_model'])) {
    ...
    if (empty($model_name) || $developer_id <= 0) {
        $error_message = "Developer and model name are required.";
    } else {
        // Check if model name already exists for this developer
        $check_stmt = $conn->prepare("SELECT id FROM project_models WHERE developer_id = ? AND name = ?");
        $check_stmt->bind_param("is", $developer_id, $model_name);
        $check_stmt->execute();
        $check_result = $check_stmt->get_result();
        
        if ($check_result->num_rows > 0) {
            $error_message = "Model name already exists for this developer.";
        } else {
            // Insert new model
            ...
            if ($insert_stmt->execute()) {
                $success_message = "Project model added successfully.";
                // Refresh the page
                header("Location: settings.php?tab=project_models");
                exit();
            } else {
                $error_message = "Error adding project model: " . $conn->error;
            }
            $insert_stmt->close();
        }
        $check_stmt->close();
...
if (isset($_POST['edit_model'])) {
    ...
    if (empty($model_name) || $developer_id <= 0) {
        ...
        // Check if model name already exists for this developer (excluding this model)
        $check_stmt = $conn->prepare("SELECT id FROM project_models WHERE developer_id = ? AND name = ? AND id != ?");
        $check_stmt->bind_param("isi", $developer_id, $model_name, $model_id);
        $check_stmt->execute();
        $check_result = $check_stmt->get_result();
        ...
        if ($check_result->num_rows > 0) {
            ...
            // Update model
            $update_stmt = $conn->prepare("UPDATE project_models SET developer_id = ?, name = ? WHERE id = ?");
            $update_stmt->bind_param("isi", $developer_id, $model_name, $model_id);
            
            if ($update_stmt->execute()) {
                $success_message = "Project model updated successfully.";
                // Refresh the page
                header("Location: settings.php?tab=project_models");
                exit();
            } else {
                $error_message = "Error updating project model: " . $conn->error;
            }
            $update_stmt->close();
        }
        $check_stmt->close();
                ...
                <div class="settings-content">
                    ...
                    <div class="settings-section">
                        <div class="settings-section-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                            <h3 class="settings-section-title" style="margin: 0;">Property Developers</h3>
    ...
    <div id="addDeveloperModal" class="modal" role="dialog" aria-labelledby="addDeveloperModalTitle" aria-hidden="true">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title" id="addDeveloperModalTitle">Add New Developer</h3>
    ...
    <div id="editDeveloperModal" class="modal" role="dialog" aria-labelledby="editDeveloperModalTitle" aria-hidden="true">
        <div class="modal-content">
            ...
            <form method="POST" action="?tab=developers">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="edit_developer_name">Developer Name</label>
