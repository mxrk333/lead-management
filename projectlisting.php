...
<div id="messenger-modal" class="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-[2000] px-4 sm:px-6 overflow-y-auto hidden backdrop-blur-sm">
  <div class="bg-white rounded-xl p-6 shadow-2xl w-full max-w-3xl relative mt-6 mb-6 overflow-y-auto max-h-[80vh] border border-gray-100">
    ...
    <select class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400">
        <option value="">Select a payment term</option>
        ${project.monthly_downpayment_3mos ? `
            <option value="3">3 Months - ${formatCurrency(project.monthly_downpayment_3mos)}</option>
        ` : ''}
        ${project.monthly_downpayment_6mos ? `
            <option value="6">6 Months - ${formatCurrency(project.monthly_downpayment_6mos)}</option>
        ` : ''}
        ${project.monthly_downpayment_12mos ? `
            <option value="12">12 Months - ${formatCurrency(project.monthly_downpayment_12mos)}</option>
        ` : ''}
        ${project.monthly_downpayment_18mos ? `
            <option value="18">18 Months - ${formatCurrency(project.monthly_downpayment_18mos)}</option>
        ` : ''}
    </select>
</div>
